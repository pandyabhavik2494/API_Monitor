...
class ClientB extends TimerTask {
	...
	ClientB( String data,String name) {
		apiURL = data;
		APIName = name;
	 }
	public void run(){
	...
	public static void main(String[] args) {
		int poll1 = 0, poll2 = 0, poll3 = 0;
				...
				if(chkAPI1){
					Timer t1 = new Timer();
					t1.schedule(new ClientB("http://services.groupkt.com/country/get","API_1"), 0, poll1*1000);
				}
				if(chkAPI2){
					Timer t2 = new Timer();
					t2.schedule(new ClientB("http://services.groupkt.com/state/get","API_2"), 0, poll2*1000);
				}
				if(chkAPI3){
					Timer t3 = new Timer();
					t3.schedule(new ClientB("https://jsonplaceholder.typicode.com/todos/1","API_3"), 0, poll3*1000);
				}
	}
}